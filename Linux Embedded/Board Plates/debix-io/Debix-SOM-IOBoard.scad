...
// Size, 145.95 x 102.00mm

// Between the fit holes: (inside to inside) 134.50 x 88.45

// The fittings are not square to the board. Who designed this!
...
module mounts(length, width, height) {
    ...
    translate([offset, offset, 0]) {
        // Resting pillars
        pillars(length, width, height, 6.5/2);
        // Fit through pillars (through hole)
        pillars(length, width, height + 2.0, 3.4/2);
    };
};

module full_plate(extra_height=2) {
    difference() {
        width=145.95;
        depth=88.45;
        union() {
            plate(width, depth, 2);
            mounts(width-6, depth-6, 6+extra_height);
        };

        // Large cut in the middle to reduce material
        // (This doesn't actually save much print time though)
        translate([10, 10, -0.01]) plate(width - 20, depth - 20, 2.2);
    };
};
